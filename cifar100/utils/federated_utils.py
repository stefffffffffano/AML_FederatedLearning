...
def client_update(model, client_id, client_data, criterion, optimizer, local_steps=4, print_log=False):
    """Client Update Function for Federated Learning

    This function trains a client's local model using their dataset for a fixed number of steps (`local_steps`). The function handles dynamic batch sizes and calculates loss and accuracy with precision to ensure robustness, especially in cases where the dataset size is not divisible by the batch size.
    ...
    Args:
        ...
        local_steps (int): Number of local steps (batches) to train on the client's dataset.
        print_log (bool): If True, logs the final loss and accuracy after training.

    Returns:
        tuple: A tuple containing the updated model's state dictionary, the averaged loss, and the accuracy.
    """
    ...
    # Return the updated model's state dictionary (weights) and mentrics
...
def evaluate(model, dataloader):
    with torch.no_grad():
        ...
        for data, targets in dataloader:
            ...
            # Calculate Accuracy
            accuracy = running_corrects / float(len(dataloader.dataset)) * 100

    return accuracy, mean(losses)


def fedavg_aggregate(global_model, client_states, client_sizes, client_avg_losses, client_avg_accuracies):
    """
...
def train_federated(global_model, criterion, trainloader, validloader, num_clients, num_classes, rounds, lr, momentum, batchsize, wd, C=0.1, local_steps=4, log_freq=10, detailed_print=False):
    ...
    for round_num in range(checkpoint_start_step, rounds):
        ...
        # 2) local training: for each client updates the model using the client's data for local_steps steps
        for client_id in selected_clients:
